...
- **You can add widgets yourself, without waiting for us.** A widget module is a `.jar` you drop into the widgets folder. The launcher finds it at startup, and there is no install step, no registry to update and no store to submit to. Nothing is sandboxed, so treat a module the way you would treat a game mod: install ones whose source you can see.
- **Browse shows more than its first page, and a pack page reads like a page.** The catalogue asked for one page and never the next, and a single refused request ended the listing until you restarted the launcher. Descriptions were drawn as one column of flat text with every image stretched to the full width, so a sixty-pixel badge came out banner-sized. Headings, tables, quotes and folds now look like what they are, screenshots have a gallery of their own, and a description can no longer hang or exhaust the launcher, which it could do from text the pack itself supplies.
...
- **A pack that is playing asks before its files are rewritten.** Update, version switch, rollback and repair all overwrite mods and configs the running game already has open, and none of them used to look at whether the game was running. They ask now and go ahead if you say so, because you may have a reason the launcher cannot see.
...
- **Nothing quietly takes your things.** Reset customization on the recovery screen deleted the notes and checklists you had typed into widgets, and asked nothing first. A launcher started while the system keyring was locked erased your saved account for good. A mod whose download failed reported itself installed. And a pack update handed back the pack's copy of a file it had never changed, over the edit you had made to it, which is how a pack that ships a server list took your server list with it.
