...
### Added
- `SmrtSource` learns the `github` variant, so a pack may pin a mod to a release asset and the launcher installs it. Sixteen entries across five packs were served out of the mirror's own cache for no reason other than that there was no way to say where they came from, which put this deployment in the business of redistributing other people's builds. The variant carries `repo`, `tag`, `asset` and the `url`: a release download is public and derivable, unlike a signed CurseForge link, so `resolveUrl` hands back what the manifest says and nothing expires. `stableKey` reads the repository and asset and not the tag, since the tag is the version and an optional mod's on/off choice has to outlive a release bump. `SourceBadge` and `NxColors.source` are exhaustive over the sealed class, so both were decided rather than defaulted; GitHub's own palette is monochrome and the token follows it, near-white on the dark ground and near-black on the light one.
