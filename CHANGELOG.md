...
### Added
- The Content tab can show only what the pack leaves up to the player. Its chips answer what kind of thing a row is -- a mod, a resource pack, a shader -- and optional is not a kind but a property of one, so it lives behind a filter control at the end of the row rather than as a fifth chip: one place for every axis that narrows within a section, and a tint while one is on, because a list quietly shorter than the folder is otherwise a mystery. Offered only where the pack's manifest actually declares optional content; a local pack curates nothing and gets no dead control. Closes #489.
