...
- The pack hero's Play button becomes a low monochrome pill -- black plate on a dark theme, white on a light one, static ink instead of the palette accent so it reads on any hero art -- and walks the launch through `IndicationCenter`: Play starts it, a dimmed pulsing wait rides prepare/sync, a running game turns it into Exit (stop), and a failure falls back to Play while the error toast carries the diagnosis. Capsule under a rounded style, the style's own hard edge under a square one, mirroring the `SwitchStyleSpec` axis split.
