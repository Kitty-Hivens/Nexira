...
- The spacing scale is derived from the code it has to describe, and starts having call sites. Its seven rungs were 2, 4, 8, 12, 16, 24 and 32, picked from an ideal; the interface meanwhile leaned on 6, 10 and 20, each of which appears in more files than two of the rungs do, with `6.dp` alone in 67 of 412 source files. A scale that cannot express what the interface already does earns no call sites, and it had exactly zero. The ladder is a rule now, steps of 2 up to twelve, steps of 4 up to twenty four, then 32, with a test that walks it so a rung added by eye fails. Rungs are named for what they measure: `Spacing.s6` says what `6.dp` said and adds the one fact the literal could not, that the value is on the scale. `:nx-ui` adopts it first, since the library owns the design and should be the first to live under it: 34 spacing sites across 15 primitives, and not one pixel moves, because every converted value was already a rung. The handful of off-ladder values inside the library stay literal for now; moving a 14dp gap to a neighbouring rung shifts layout, which is a decision separate from adopting a scale.
