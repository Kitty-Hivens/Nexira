...
### Removed
- The experimental per-role colour and shape override layer is removed -- `CustomizationSettings.experimentalColorOverridesEnabled` / `colorOverrides` / `styleOverrides`, the `ColorRole` and `StyleOverrides` types, `StyleSpec.applyOverrides`, and the override block in `NxTheme`. It predated the modular widget system and painted one global set of colours and shapes over the whole theme, overriding per-widget styling instead of letting each widget own its look.
- The standalone Customization screen goes with it -- `Screen.CustomizationExtension`, `CustomizationSurface` and its density / glass-intensity / accent / reset widgets, the `AppearanceSection` entry row, the `BreadcrumbResolver` and `EditorSurfaceHost` cases, and the `customization` surface in `default-layout.json`. `CustomizationSettings` stays as the live store the render stack reads (density, glass intensity, accent override) and the editor's `SurfacePropertiesPanel` writes (nav-rail selection); the dropped knobs keep applying at their stored values, their editing to return per-widget.
