...
- A frosted surface blurs what is actually beneath it. It used to be answered by redrawing the wallpaper at the surface's own offset, which is a narrower question than the one asked: a plane over another plane showed the wallpaper it could not see rather than the plane it covered, and the redraw itself was measured drawing nothing at all for any surface away from the window origin. Skia's `saveLayer` with a backdrop filter does it in one call in the draw phase, exposed through skiko, and needs no knowledge of what it is blurring. A surface that names no radius takes `StyleSpec.surfaceBlur`, so the form axis decides: Celestia blurs at 18dp, Brut does not blur at all, and there is no per-call-site switch to keep in step with the style. Two limits are inherent rather than defects: a backdrop filter reads the current layer, so an ancestor with alpha below 1 isolates the element and the filter finds it empty (scale, rotation and clipping do not isolate, only alpha does), and the result cannot be cached because the destination it reads may change on any frame.
...
- What a wallpaper is allowed to be is decided by the decoders instead of by a list of extensions. `backgroundMediaKind` asks Skia how many frames it finds and hands anything Skia will not open at all to the player, whose bundle carries every decoder and demuxer FFmpeg builds without an external library, so avi, MPEG-TS, wmv, flv and the rest of the long tail now play where the old list did not name them and they fell through to the image path, which drew nothing and logged a decode failure. A GIF holding one frame takes the still path now, which caches and downscales it, rather than a decode thread. The file dialog still offers a set of extensions, as a way to find the file rather than as the boundary of what is accepted. A gallery or banner URL still decides by extension because that decision is made before there are any bytes to ask about, and the set it matches covers what the natives read rather than the containers the mirror happened to ship first.
