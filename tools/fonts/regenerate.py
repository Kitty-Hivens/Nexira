...
COVERAGE_OUT = os.path.join(ROOT, "nx-ui", "src", "desktopMain", "kotlin", "hivens", "ui", "text", "UiFaceCoverage.kt")
...
def write_coverage_table():
    """Emits the UI face's own character table as Kotlin."""
    cmap = TTFont(UI_FACE).getBestCmap()
    runs = runs_of(cmap)
    lines = [
        ...
        f" * start/end pairs. {len(cmap)} codepoints in {len(runs)} runs.",
        " */",
