...
object DiagnosticBundle {
    ...
    fun create(paths: PlatformPaths): Path {
        ...
        ZipOutputStream(Files.newOutputStream(output)).use { zip ->
            ...
            // Crash reports — copy everything in the dir verbatim (they were already
            // generated through CrashReporter which doesn't write secrets).
            ...
            if (Files.exists(crashDir)) {
                Files.list(crashDir).use { stream ->
                    stream.filter { Files.isRegularFile(it) }.forEach { f ->
                        writeText(zip, "crash-reports/${f.fileName}", Files.readString(f))
                    }
                }
            }
        }
