...
Непрерывная интеграция на пул-реквесте гоняет восемь наборов: `client-config`, `client-core`, `client-launcher`, `client-update`, `client-ui`, `client-i18n`, `client-render3d` и `widget-processor` на Linux, macOS и Windows. Дизайн-система, модель и рантайм виджетов, модули авторизации, CLI, медиа, трей и два агента там не запускаются.
