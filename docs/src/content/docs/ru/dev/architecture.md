...
### Платформенные директории данных

Пользовательские данные (настройки, креды, скачанные клиенты, кэш скинов, логи, crash-репорты) живут в одной директории на ОС, которую резолвит `PlatformPaths` в `client-launcher`:
...
При первом запуске legacy-директория `~/.aura/` (использовалась в версиях ≤ 2.2.x) копируется в новое место, в неё пишется маркер `.migrated`. Старые данные **не удаляются** — пользователь может убрать их вручную, убедившись что миграция прошла.
