...
Шестнадцать модулей Gradle. Направление зависимостей одностороннее и несущее: движок не видит интерфейс, а дизайн-система не видит предметную область.
...
Непрерывная интеграция на пул-реквесте гоняет три набора (`client-config`, `client-core`, `client-launcher`) на Linux, macOS и Windows. Интерфейс, дизайн-система, виджетные модули и модули авторизации там не запускаются.

Два собственных сканера при этом работают строго на каждом пул-реквесте: один падает на строке, показанной пользователю в обход слоя локализации, второй на процессных метаданных в комментариях. Оба сканируют движок и `client-ui`, ни один не сканирует `nx-ui` и виджетные модули.
