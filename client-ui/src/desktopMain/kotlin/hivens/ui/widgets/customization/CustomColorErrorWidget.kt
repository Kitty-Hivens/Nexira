...
fun CustomColorErrorWidget(instance: WidgetInstance) {
    ...
    ColorRoleRow(
        ...
        currentHex   = settings.colorOverrides[ColorRole.ERROR],
        invalidLabel = s.customizationHexInvalid,
        onValidHex   = { hex -> ctx.update { copy(colorOverrides = colorOverrides + (ColorRole.ERROR to hex)) } },
        onClear      = { ctx.update { copy(colorOverrides = colorOverrides - ColorRole.ERROR) } },
    )
}
